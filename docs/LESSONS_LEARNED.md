...
## 2024-10-27: Implementation of Cognitive Rheology & Variable Viscosity Prompting

*   **Context:** The standard prompting paradigm treats the LLM as a static interlocutor, leading to Interpretive Fracture (decay of structural intent over long contexts) and the Sisyphus Loop (circular token repetition). We needed a way to dynamically control the "flow" of inference based on real-time thermodynamic metrics.
*   **Action:** Architected the Rheological Mode Switcher (RMS) as a Layer-1 meta-architectural component. We replaced static temperature parameters across all services (e.g., `geminiService`, `featureDiscoveryAgent`) with dynamic telemetry evaluation via the `executeGenerativeTask` executor.
*   **Learning:** By treating inference as a non-Newtonian fluid, we can autonomously switch between Crystal Mode ($T \approx 0$, rigid schemas) during semantic entropy spikes, and Cloud Mode ($T > 0.7$) when repetition loops are detected. This prevents the LLM from reverting to its pre-trained mean during highly deterministic, structural tasks.
*   **Adoption:** The Rheological Controller now governs all generative acts through `services/cognitiveExecutor.ts`. The implementation and its cybernetic mechanisms are fully documented in `DOMAIN_GLOSSARY.md` and `PROBLEM_SPACES_AND_SOLUTIONS.md`.
